namespace YourCompany.Bank.DirectDebit;

using Microsoft.Bank.Statement;
using System.IO;

codeunit 56130 "Imp SEPA CAMT PostMap YCO"
{
    ...
    begin
        // New file format: additional transaction info at TxDtls level
        InsertRelatedPartyInformation(
            Rec,
            '/Document/BkToCstmrStmt/Stmt/Ntry/NtryDtls/TxDtls/AddtlTxInf',
            CBGStatementLineAddInfo."Information Type"::"Description and Sundries",
            true,
            true);

        // Optional fallback for older files (keep or remove as needed)
        InsertRelatedPartyInformation(
            Rec,
    ...
    begin
        ...
        ColumnNo := FindColumnNoOfPath(DataExchLineDef."Data Exch. Def Code", DataExchLineDef.Code, XmlPath);
        if ColumnNo = 0 then
            exit;
    ...
    local procedure FindColumnNoOfPath(DataExchDefCode: Code[20]; DataExchLineDefCode: Code[20]; XmlPath: Text): Integer
    var
    ...
    begin
        ...
        DataExchColumnDef.SetRange(Path, XmlPath);
